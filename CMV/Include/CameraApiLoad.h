...
CameraSdkStatus LoadSdkApi()
{
	...
	if (ERROR_SUCCESS != RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\Industry Camera", 0, KEY_READ, &hkey))
...
#ifdef _WIN64
	sprintf(strPath, "Software\\%s\\Digital Camera Platform\\Settings_X64", szCompany);
#else
	sprintf(strPath, "Software\\%s\\Digital Camera Platform\\Settings", szCompany);
#endif
	hkey = NULL;
	RegCreateKeyEx(HKEY_CURRENT_USER, strPath, 0, NULL, 0, KEY_ALL_ACCESS, NULL, &hkey, NULL);
